			...
			/* 	pageMake(res);
				...
				$.each(res, function(idx, item){
					...
					str+="<td role='gridcell' style='height: 0px; width: 7%;'><form name='frmCart' method='post' action='<c:url value='/registration/open_registrationCart'/>'><input type='hidden' name='openSubCode' value='"+item.OPEN_SUB_CODE+"'> <input type='submit' id='btCart' value='장바구니' class='buttons'> </form> </td>"; //장바구니
						...
						str+="<td role='gridcell' style='height: 0px; width: 9%;'><form name='frmSyllabus' method='get' action='<c:url value='/registration/download'/>'><input type='hidden' name='openSubCode' value='"+item.OPEN_SUB_CODE+"'> <input type='submit' id='btSylla' value='강의계획서' class='buttons'> </form> </td>";
					str+="</tr>"; //테이블 닫는거
				});
